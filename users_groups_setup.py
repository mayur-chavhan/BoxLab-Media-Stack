...
class UserGroupSetup:
    ...
    def create_config_dir(self, service_name, owner=None):
        ...
        os.system(
            '/bin/bash -c "sudo mkdir -p ' + self.root_dir + '/config/' + service_name + '-config -m 775'
            ' ; sudo chown -R ' + owner_name + ':mediacenter ' + self.root_dir + '/config/' + service_name + '-config'
            ' ; sudo chown $(id -u):mediacenter ' + self.root_dir + '/config"'
        )
    ...
    def kapowarr(self):
        os.system(
            '/bin/bash -c "sudo useradd kapowarr -u 13015'
            ' ; sudo mkdir -pv ' + self.root_dir + '/data/temp_downloads -m 775'
            ' ; sudo chown -R kapowarr:mediacenter ' + self.root_dir + '/data/temp_downloads"'
        )
        self.create_config_dir('kapowarr')
        os.system('sudo usermod -a -G mediacenter kapowarr')

    def autobrr(self):
        os.system('/bin/bash -c "sudo useradd autobrr -u 13016"')
        self.create_config_dir('autobrr')
        os.system('sudo usermod -a -G mediacenter autobrr')

    def tdarr(self):
        os.system(
            '/bin/bash -c "sudo useradd tdarr -u 13017'
            ' ; sudo mkdir -pv ' + self.root_dir + '/config/tdarr/{server,configs,logs} -m 775'
            ' ; sudo mkdir -pv ' + self.root_dir + '/data/transcode -m 775'
            ' ; sudo chown -R tdarr:mediacenter ' + self.root_dir + '/config/tdarr'
            ' ; sudo chown -R tdarr:mediacenter ' + self.root_dir + '/data/transcode'
            ' ; sudo chown $(id -u):mediacenter ' + self.root_dir + '/config"'
        )
    ...
    def wizarr(self):
        os.system(
            '/bin/bash -c "sudo useradd wizarr -u 13018'
            ' ; sudo mkdir -pv ' + self.root_dir + '/data/wizarr-cache -m 775'
            ' ; sudo chown -R wizarr:mediacenter ' + self.root_dir + '/data/wizarr-cache"'
        )
        self.create_config_dir('wizarr')
        os.system('sudo usermod -a -G mediacenter wizarr')

    def decluttarr(self):
        os.system('/bin/bash -c "sudo useradd decluttarr -u 13019"')
        self.create_config_dir('decluttarr')
        os.system('sudo usermod -a -G mediacenter decluttarr')

    def janitorr(self):
        os.system(
            '/bin/bash -c "sudo useradd janitorr -u 13020'
            ' ; sudo mkdir -pv ' + self.root_dir + '/data/janitorr/logs -m 775'
            ' ; sudo chown -R janitorr:mediacenter ' + self.root_dir + '/data/janitorr"'
        )
    ...
    def profilarr(self):
        os.system(
            '/bin/bash -c "sudo mkdir -pv ' + self.root_dir + '/config/profilarr-config -m 775'
            ' ; sudo chown -R ' + self.local_user + ':mediacenter ' + self.root_dir + '/config/profilarr-config'
            ' ; sudo chown $(id -u):mediacenter ' + self.root_dir + '/config"'
        )

    def jellystat(self):
        os.system(
            '/bin/bash -c "sudo mkdir -pv ' + self.root_dir + '/data/jellystat/{postgres-data,postgres-backup} -m 775'
            ' ; sudo chown -R ' + self.local_user + ':mediacenter ' + self.root_dir + '/data/jellystat"'
        )
