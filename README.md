...
BoxLab Media Server Stack is a curated fork of the excellent Ezarr project with the additional
applications that power the "Ultimate Jellyfin Stack". You still get the battle-tested Ezarr CLI,
folder layout, and hardlink-friendly defaults, but now with optional access to all of the extra
...
- [Kapowarr](https://github.com/mRcS-IT/kapowarr) keeps up with your comic libraries and can feed the same download clients.
- [Tdarr](https://github.com/haveagitgat/Tdarr) automates media transcoding and pruning.
- [Autobrr](https://autobrr.com/) joins tracker IRC channels so you can grab torrents immediately.
...
- [Decluttarr](https://github.com/manimatter/decluttarr) keeps your Sonarr/Radarr/Lidarr queues clean and removes stale downloads.
...
- [Janitorr](https://github.com/schaka/janitorr) cleans up untagged media and stale library entries.
- [Profilarr](https://github.com/Dictionarry-Hub/profilarr) manages per-instance quality profiles and custom formats from a single UI.

> **Heads-up:** Gluetun is shipped as a standalone VPN container. If you want to tunnel qBittorrent, Prowlarr, Autobrr, etc. you still need to edit the generated compose file and attach those services via `network_mode: "service:gluetun"`.
...
1. To get started, clone the repository in a directory of your choosing. `git clone https://github.com/<your-user>/boxlab-media-server-stack.git`
...
API key for all of them. This would lead to everyone using BoxLab (or upstream Ezarr) having the same API key
and user/password combination. Personally, I'd rather trust users to figure this out on their own rather
